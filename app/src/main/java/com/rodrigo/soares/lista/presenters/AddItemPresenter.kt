...
class AddItemPresenter(val activity: AddItemActivity) {
    ...
    fun saveItem(connection: DBConnection, selectedList: Lista){
        ...
        itemDao.save(Item(activity.etItemName.text.toString(),
                          activity.etItemPrice.text.toString().toDouble(),
                          selectedList.id,
                          activity.etItemLink.text.toString(),
                          itemDao.getQtByIdLista(selectedList.id!!)))
        activity.finish()
    }
}